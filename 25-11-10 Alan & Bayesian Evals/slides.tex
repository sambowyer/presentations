...
\begin{frame}{Limitations of Importance Sampling (IS)}
  \begin{itemize}[<+->]
    ...
    \begin{itemize}
      \item $z = (z_1, \ldots, z_n) \in \mathcal{Z}$.
...
\begin{frame}{Bayesian Evals for Interpretability}
  \begin{figure}
  \includegraphics[width=\textwidth, height=0.8\textheight, keepaspectratio]{img/bayes_evals/fig/pretrained_bbh_subset.pdf}
  \end{figure}
  \pause 
...
\begin{frame}{Bayesian Evals for Interpretability}
  \begin{itemize}[<+->]
    \item Given the eval question and reponse for an LLM, use an SAE-like model to enforce sparsity on features across the latent dimensions.
  \end{itemize}
  \vspace{-0.2cm}
  \pause
  \begin{figure}
...
\begin{frame}{Future Directions}
  \begin{itemize}[<+->]
    \item The results were interesting, but not as straightforwardly interpretable as we hoped.
    \item Perhaps these techniques could be used more successfully for predicting eval performance based on something like pretraining metrics?
  \end{itemize}
\end{frame}
