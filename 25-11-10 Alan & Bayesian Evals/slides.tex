...
\begin{frame}{MP-IS: Some More Complications...}
  ...
  \begin{itemize}[<+->]
    \item At first glance, this thing doesn't look all that nice to compute...
...
\begin{frame}{MP-IS: Some More Complications...}
  ...
  \end{itemize}
    ...
    $$
    % \begin{aligned}
      \mathcal{P}_\text{MP}(z) = \frac{1}{K^3} \sum_{k_1 \in [K]} \sum_{k_2 \in [K]} \sum_{k_3 \in [K]} \frac{P(z_1^{k_1}) P(z_2^{k_2} | z_1^{k_1}) P(z_3^{k_3} | z_2^{k_2}) P(x | z_3^{k_3})}{Q_\text{MP}(z^\mathbf{k}, \mathbf{k})} 
    $$
    \pause
    $$
      = \frac{1}{K^3} \sum_{k_1 \in [K]} \underbrace{\frac{P(z_1^{k_1})}{Q_\text{MP}(z_1^{k_1}, k_1)}}_{\text{Vector of size } K} 
      \sum_{k_2 \in [K]} \underbrace{\frac{P(z_2^{k_2} | z_1^{k_1})}{Q_\text{MP}(z_2^{k_2}, \mathbf{k}_{1:2})}}_{\text{Matrix of size } K \times K}
      \sum_{k_3 \in [K]} \underbrace{\frac{P(z_3^{k_3} | z_2^{k_2})}{Q_\text{MP}(z_3^{k_3}, \mathbf{k}_{2:3})}}_{\text{Matrix of size } K \times K}
      \underbrace{\frac{P(x | z_3^{k_3})}{Q_\text{MP}(x | z_3^{k_3}, k_3)}}_{\text{Vector of size } K}
    % \end{aligned}
    $$
...
\begin{frame}{MP-VI}
  \begin{itemize}[<+->]
    ...
    \item Aitchison (2019) showed that MP-VI is a tighter bound than the global VI objective (IWAE):
      $$
      \log P(x) \geq \mathcal{L}_\text{global}(\theta) = \mathbb{E}_{z \sim Q_\theta}[\log \mathcal{P}_\text{global}(z)]
      $$
  \end{itemize}
  \pause
  \begin{figure}
...
\begin{frame}{MP Algorithms}
  \begin{itemize}[<+->]
    \item We showed that we can obtain unbiased posterior moment estimates efficiently using autodiff (Bowyer et al. (2024)).
      $$
    ...
    \item We define a modified marginal likelihood estimator with an auxiliary variable $J \in \mathbb{R}$:
      ...
      $$
    \item Then differentiating with respect to $J$ and setting $J=0$ we get:
      $$
      \frac{\partial}{\partial J}\bigg|_{J=0} \log \mathcal{P}_\text{MP}^\text{exp} (z, J) = m_\text{MP}(z)
      $$
    \item Through a similar argument, we can set $J$ to be vector- or tensor-valued based on $K$ and the shape of $z$, allowing us to compute marginal likelihoods and importance samples too.
  \end{itemize}
  \vspace{0.1cm}
\end{frame}
...
\begin{frame}{QEM: An Adaptive Importance Sampling Algorithm}
  \begin{minipage}{0.4\textwidth}
    \begin{itemize}
      \item \textbf{QEM:} use these posterior moment estimates to iteratively update the approximate posterior $Q_\text{MP}$ in an EM-like algorithm for adaptive imortance sampling (Heap et al. (2025)).
      % \item This algorithm is called QEM (Expectation-Maximisation for Approximate Posteriors).
    \end{itemize}    
  \end{minipage}
...
\begin{frame}{Bayesian Evals: Uncertainty Quantification for LLM Evals}
  ...
  \begin{itemize}[<+->]
    \item Two directions:
      ...
      \end{itemize}
    \item The former direction led to an ICML spotlight position paper.%: `Position: Don't Use the CLT in LLM Evals With Fewer Than a Few Hundred Datapoints'.
...
\begin{frame}{Motivation}
  ...
  \begin{itemize}[<+->]
      ...
      \item In the case of binary data, the CLT-based confidence interval is:
      $$
        \text{CI}_{1-\alpha}(\theta) = \bar{X} \pm z_{\alpha/2} \sqrt{\frac{\bar{X}(1-\bar{X})}{N}}
      $$
      where $z_{\alpha/2}$ is the $100(1-\alpha/2)$-th percentile of $\mathcal{N}(0, 1)$.
  \end{itemize}
...
\begin{frame}{Real-World Failures of the CLT}
  ...
  \begin{minipage}{0.45\textwidth}
    ...
    \begin{figure}
      ...
      \caption{Math Arena's AIME II 2025 Benchmark (N=15)}
    \end{figure}
  \end{minipage}
  \begin{minipage}{0.45\textwidth}
    \centering
...
\begin{frame}{Interval Comparison Simulations}
  ...
  \begin{itemize}[<+->]
      ...
      \item Repeat many times and calculate the true coverage and width of the intervals.
      \begin{itemize}
...
\begin{frame}{IID Questions Setting: Results}
  \begin{figure}
      \includegraphics[width=\textwidth, height=0.8\textheight, keepaspectratio]{img/bayes_evals/pngs/exp4-1.png}
...
\begin{frame}{Other Eval Settings}
  \begin{itemize}[<+->]
      ...
      \begin{itemize}
          \item We have $T$ tasks, each with $N_t$ IID questions, instead of $N$ IID questions.
      \end{itemize}
...
\begin{frame}{Clustered Questions Setting: A Bayesian Approach}
  \begin{itemize}[<+->]
    ...
    \theta \sim \text{Beta}(1, 1) = \text{Uniform}[0, 1].
    $$
    \item $\theta_t$ is the difficulty of the questions in task $t$ (we ensure that $\mathbb{E}[\theta_t] = \theta$),
    $$  
    \theta_t \sim \text{Beta}(d \theta, d (1-\theta)).
    $$
    \item If $d$ is small, then $\theta_t$ is close to $\theta$ for all tasks.
    \item If $d$ is large, then $\theta_t$ is more variable across tasks.
  \end{itemize}
\end{frame}
...
\begin{frame}{Conclusion}
  ...
  \begin{itemize}[<+->]
      \item Use \alert{Bayesian Beta-Bernoulli} or \alert{Wilson Score intervals}.
      \item It's \textbf{simple} (use \texttt{scipy} or \texttt{bayes\_evals}).
